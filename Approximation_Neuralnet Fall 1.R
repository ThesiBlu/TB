...
nn1 <- neuralnet(y1 ~ x1, data = dat1, hidden = c(2,2,2,2), rep=1, err.fct = "sse")
plot(nn1, information = FALSE, show.weights = FALSE)

nn2 <- neuralnet(y2 ~ x2, data = dat2, hidden = c(2,2,2,2), rep=1, err.fct = "sse")
plot(nn2)

nn3 <- neuralnet(y3 ~ x3, data = dat3, hidden = c(2,2,2,2), rep=1, err.fct = "sse")
plot(nn3)

nn4 <- neuralnet(y4 ~ x4, data = dat4, hidden = c(2,2,2,2), rep=1, stepmax = 1e+08)
plot(nn4)

nn5 <- neuralnet(y5 ~ x5, data = dat5, hidden = c(2,2,2,2), rep=1, stepmax = 1e+08)
plot(nn5)

nn6 <- neuralnet(y6 ~ x6, data = dat6, hidden = c(2,2,2,2), rep=1, stepmax = 1e+08)
plot(nn6)
...
fall1_matrix <-  cbind(nn1$result.matrix,nn2$result.matrix,nn3$result.matrix,nn4$result.matrix,nn5$result.matrix,nn6$result.matrix)

#Fehlertabelle
Error <- fall1_matrix[1,]
